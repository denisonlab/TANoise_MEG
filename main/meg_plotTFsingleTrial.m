...
foi            = 1:50;
t_ftimwin      = 10 ./ foi;
toi            = p.tstart/1000:0.001:p.tstop/1000;
tfAmps = [];
...
for iC = 1:nConds
    ...
    for iCh = selectedChannels
        ...
        spectrumTrials(:,:,:,count) = abs(spectrum); % trials x time (10ms) x freq x ch
        count = count+1; 
    ...
    meanAmpsSingleTrial = mean(spectrumTrials,4,'omitnan'); 
    powSingleTrial = spectrumTrials.^2; 
    meanPowSingleTrial = mean(powSingleTrial,4,'omitnan'); 
    ...
    A.(cond).ampsSingleTrial = spectrumTrials;
    A.(cond).powSingleTrial = powSingleTrial; 
    A.(cond).meanAmpsSingleTrial = meanAmpsSingleTrial;
    A.(cond).meanPowSingleTrial = meanPowSingleTrial; 
...
% ylabel('amp')
% xlabel('time (s)')
% xline(p.eventTimes/1000,'k',p.eventNames)
% legend(condNames,'Location','southwest')
