...
% -- Export CSV --- 
exportCSV = 1; 
user = 'kantian'; % 'kantian'
csvDir = sprintf('/Users/%s/Dropbox/Data/TANoise/TANoise-stats/data',user);
if ~exist(csvDir, 'dir')
    mkdir(csvDir)
end
...
%% Plot group (phase) by amplitude --- Sessions --- 
figure
set(gcf,'Position',[100 100 800 200])
...
%% Plot precue T1 phase distribution 95% CI same fig 
figure
...
%% Plot precue T2 phase distribution 95% CI same fig 
figure
set(gcf,'Position',[100 100 240 190])

% --- Plot precue T2 ---
val = circ_dist(fittedP.linear2Hz.valR2_min(:,1),fittedP.linear2Hz.valR2_min(:,2)); % average sessions  
polarhistogram(val,edges,'FaceColor',p.cueColors(2,:),'EdgeColor',p.cueColors(2,:),'FaceAlpha',1); % fitted phase histogram
hold on 
[mu ul ll] = circ_mean(val); 
r = circ_r(val); 
...
if saveFigs
    figTitle = sprintf('Group_TANoise_ITPCFit_DataPrecue_%s_Linear2Hz_Phase_Summary_sessions%d_PrecueT2_Diff',...
        mdlFitType,...
        nSubs/2);
    saveas(gcf,sprintf('%s/%s.svg', figDir, figTitle))
end
...
%% Plot session 1 to session 2 difference by precue per subject 
figure
set(gcf,'Position',[100 100 240 190])
...
% --- Plot precue T2 ---
% val = circ_dist(fittedP.linear2Hz.valR2_min(:,1),fittedP.linear2Hz.valR2_min(:,2)); % difference in fitted phase between sessions 1 and 2  
% polarhistogram(val,edges,'FaceColor',p.cueColors(2,:),'EdgeColor',p.cueColors(2,:),'FaceAlpha',1); % fitted phase histogram
% hold on 
% [mu ul ll] = circ_mean(val); 
% r = circ_r(val); 
...
if saveFigs
    figTitle = sprintf('Group_TANoise_ITPCFit_DataPrecue_%s_Linear2Hz_Phase_Summary_sessions%d_PrecueT1T2_DifferenceBySession',...
        mdlFitType,...
        nSubs/2);
    saveas(gcf,sprintf('%s/%s.svg', figDir, figTitle))
end
...
% --- Plot precue T1 - precue T2 ---
val = circ_dist(fittedP.linear2Hz.valR_diffMin(:,1),fittedP.linear2Hz.valR_diffMin(:,2)); % difference between precues between sessions 
polarhistogram(val,edges,'FaceColor',[0.7 0.7 0.7],'EdgeColor',[0.7 0.7 0.7],'FaceAlpha',0.7); % fitted phase histogram
hold on 
[mu ul ll] = circ_mean(val); 
r = circ_r(val); 
...
if saveFigs
    figTitle = sprintf('Group_TANoise_ITPCFit_DataPrecue_%s_Linear2Hz_Phase_Summary_sessions%d_PrecueT1T2Diff_DifferenceBySession',...
        mdlFitType,...
        nSubs/2);
    saveas(gcf,sprintf('%s/%s.svg', figDir, figTitle))
end
