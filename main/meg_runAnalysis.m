...
paramType = 'Analysis'; % 'ITPCsession8'; % for meg_params 'Preproc', 'Analysis', 'ITPC', 'ITPCsession8" (to correct for the cutoff run)
analType = 'TF'; % 'ITPC'; % 'none','readdata','sortchannels','ERF','TF','decode', 'TFwholeTrial' 'TFsingleTrial'
avgTrial = 1; % 0 for single trial, 1 for average trial
sliceType = 'cue'; % 'all','cue','cueAcc','ITIjitter','ITICue'

% data
getData = 'fromMat'; % 'fromSqd' (read data from sqd), 'fromMat' (load data from prepared .mat, only time 4401) 
saveData = 0; % save .mat 
...
selectChannels = 1;
nChannelsSelected = 157; % 5, number of channels to select from channelsRanked
...
switch analType
    ...
    case 'TFsingleTrial'
        ...
        if saveAnalysis
            save(sprintf('%s/TFspectrogram_5Ch_singleTrial.mat',matDir),'A','-v7.3')
        end
