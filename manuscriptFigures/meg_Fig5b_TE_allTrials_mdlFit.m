function meg_Fig5b_TE_allTrials_mdlFit
% Plot ITPC all trials with model fits (linear, and linear+periodic)
% Fig5b

%% Load data
filename = 'figData/ModelFit_freeFreq_group.mat'; 
load(filename)
...
%% Plot figure
for iF = 2 % (Periodic model only) 1:numel(fitTypes)
    for iS = 1:size(data.all.(fitLevel),2) % sessions
        ...
        % --- Save fig ---
        if saveFigs
            figTitle = sprintf('Fig5b_TE_allTrials_mdlFit_freeFreq'); 
