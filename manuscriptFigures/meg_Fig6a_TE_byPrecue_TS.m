function meg_Fig6a_TE_byPrecue_TS
% function meg_Fig6a_TE_byPrecue_TS
% Plot 2 Hz model fit 

%% Load data
% Model fit data 
filename = 'figData/ModelFit_SeparatePrecueT1T2_2Hz_GroupTS.mat';
% ITPC data 
load(filename)
...
for iF = 1:numel(fitTypes)
    for iS = 1:size(data.all.(fitLevel),2) % sessions
        ...
        for iC = 1:numel(cueLevel) % Plot again to be on top
            ...
            plot(t,yhat,'--','LineWidth',2,'Color',colors.(cDark))
        end
        ...
        % --- Save fig ---
        if saveFigs
            figTitle = sprintf('Fig6a_TE_byPrecue_mdlFit_%s',fitTypes{iF}); 
